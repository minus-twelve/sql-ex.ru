# **sql-ex.ru**

## 1. Найдите номер модели, скорость и размер жесткого диска для всех ПК стоимостью менее 500 дол. Вывести: model, speed и hd
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/1.sql)


## 2. Найдите производителей принтеров. Вывести: maker
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/2.sql)


## 3. Найдите номер модели, объем памяти и размеры экранов ПК-блокнотов, цена которых превышает 1000 дол.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/3.sql)


## 4. Найдите все записи таблицы Printer для цветных принтеров.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/4.sql)


## 5. Найдите номер модели, скорость и размер жесткого диска ПК, имеющих 12x или 24x CD и цену менее 600 дол.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/5.sql)


## 6. Для каждого производителя, выпускающего ПК-блокноты c объёмом жесткого диска не менее 10 Гбайт, найти скорости таких ПК-блокнотов. Вывод: производитель, скорость.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/6.sql)


## 7. Найдите номера моделей и цены всех имеющихся в продаже продуктов (любого типа) производителя B (латинская буква).
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/7.sql)


## 8. Найдите производителя, выпускающего ПК, но не ПК-блокноты.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/8.sql)


## 9. Найдите производителей ПК с процессором не менее 450 Мгц. Вывести: Maker
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/9.sql)


## 10. Найдите модели принтеров, имеющих самую высокую цену. Вывести: model, price
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/10.sql)


## 11. Найдите среднюю скорость ПК.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/11.sql)


## 12. Найдите среднюю скорость ПК-блокнотов, цена которых превышает 1000 дол.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/12.sql)


## 13. Найдите среднюю скорость ПК, выпущенных производителем A.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/13.sql)


## 14. Найдите класс, имя и страну для кораблей из таблицы Ships, имеющих не менее 10 орудий.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/14.sql)


## 15. Найдите размеры жестких дисков, совпадающих у двух и более PC. Вывести: HD
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/15.sql)


## 16. Найдите пары моделей PC, имеющих одинаковые скорость и RAM. В результате каждая пара указывается только один раз, т.е. (i,j), но не (j,i), Порядок вывода: модель с большим номером, модель с меньшим номером, скорость и RAM.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/16.sql)


## 17. Найдите модели ПК-блокнотов, скорость которых меньше скорости каждого из ПК. Вывести: type, model, speed
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/17.sql)


## 18. Найдите производителей самых дешевых цветных принтеров. Вывести: maker, price
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/18.sql)


## 19. Для каждого производителя, имеющего модели в таблице Laptop, найдите средний размер экрана выпускаемых им ПК-блокнотов. Вывести: maker, средний размер экрана.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/19.sql)


## 20. Найдите производителей, выпускающих по меньшей мере три различных модели ПК. Вывести: Maker, число моделей ПК.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/20.sql)


## 21. Найдите максимальную цену ПК, выпускаемых каждым производителем, у которого есть модели в таблице PC. Вывести: maker, максимальная цена.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/21.sql)


## 22. Для каждого значения скорости ПК, превышающего 600 МГц, определите среднюю цену ПК с такой же скоростью. Вывести: speed, средняя цена.
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/22.sql)


## 23. Найдите производителей, которые производили бы как ПК со скоростью не менее 750 МГц, так и ПК-блокноты со скоростью не менее 750 МГц. Вывести: Maker
[Answer](https://github.com/minus-twelve/sql-ex.ru/blob/main/Answers/23.sql)
